...
class RmClient extends \Redmine\Client
{
     ...
     * @var bool
     */
    protected $execCacheEnabled = true;
    ...
    public function __construct( $url, $apikeyOrUsername, $pass = null, $fileCacheLifetime = 3600, $projectDir = './' )
    {
        ...
        $this->url       = $url;
        $this->fileCache = new FilesystemAdapter( md5( $this->url ), $fileCacheLifetime, $projectDir . '/var/cache/redmine' );
     ...
     * @param bool $mem
     */
    public function enableCache( bool $exec = true, bool $file = true ): RmClient
    {
        $this->execCacheEnabled = (bool)$exec;
    ...
    public function get( $path, $decode = true )
    {
        ...
        // Take from mem cache
        if ( $this->execCacheEnabled && isset( $this->execCache[ $cacheHash ] ) ) {
            $this->console( '<fg=magenta>[RDM] MEM ' . $path . '</>', OutputInterface::VERBOSITY_DEBUG );

            return $this->execCache[ $cacheHash ];
        }

        // Take from file cache
        if ( $this->fileCacheEnabled ) {
            $fileCacheItem = $this->fileCache->getItem( $cacheHash );
            if ( $fileCacheItem->isHit() ) {
                $this->console( '<fg=magenta>[RDM] FIL ' . $path . '</>', OutputInterface::VERBOSITY_DEBUG );
                return $this->execCache[ $cacheHash ] = $fileCacheItem->get();
            }
        }
        ...
        // Store in cache
        if ( $this->execCacheEnabled ) {
            $this->execCache[ $cacheHash ] = $return;
        }
        if ( $this->fileCacheEnabled ) {
            $fileCacheItem->set( $return );
            $this->fileCache->save( $fileCacheItem );
        }
